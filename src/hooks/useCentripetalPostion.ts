...
export function useCentripetalPostion() {
  const { fontSize, palaceSide, palaceStrokeWidth } = SimulatorContainer.useContainer();
  return useMemo(
    () => [
      // 0
      {
      ...
      // 4
      {
        ...
        text: {
          x: palaceSide * 0.5 + palaceStrokeWidth * 2 + fontSize * 1.2,
          y: palaceSide + palaceStrokeWidth * 2 + fontSize,
        },
      },
      // 5
